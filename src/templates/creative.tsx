...
export function CreativeTemplate({ resume }: TemplateProps) {
  ...
  const SH = ({ title }: { title: string }) => (
    <div className="mb-3 flex items-center gap-3">
      <div className="h-6 w-6 rotate-45 shrink-0" style={{ backgroundColor: PRIMARY }} />
    ...
    <div className="m-0 p-0 w-full flex-1 bg-white" style={{ fontFamily: 'Inter, sans-serif' }}>
      <div className="relative overflow-hidden px-8 py-8 text-white" style={{ background: `linear-gradient(135deg, ${PRIMARY}, ${ACCENT})` }}>
        ...
        <div className="relative flex items-center gap-5">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-white">{pi.fullName}</h1>
              ...
              {resume.experience.map(exp => (
                <div key={exp.id} className="border-l-2 pl-4" style={{ borderColor: ACCENT }}>
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold text-white" style={{ backgroundColor: PRIMARY }}>{exp.period}</span>
                  </div>
          ...
          <div className="mb-6" data-section>
            ...
            <div className="space-y-2">
              {resume.skills.map(skill => (
                <div key={skill.id}>
                  <p className="mb-1 text-xs font-bold uppercase tracking-wider" style={{ color: PRIMARY }}>{skill.category}</p>
          ...
          <div className="mb-6" data-section>
            ...
            <div className="space-y-3">
              {resume.projects.map(proj => (
                <div key={proj.id} className="rounded-lg p-3" style={{ border: `1px solid ${ACCENT}30`, backgroundColor: `${PRIMARY}05` }}>
                  <div className="flex items-baseline justify-between">
                  ...
                  {proj.technologies.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {proj.technologies.map((t, i) => <span key={i} className="rounded-full px-2 py-0.5 text-[10px] font-medium text-white" style={{ backgroundColor: PRIMARY }}>{t}</span>)}
                    </div>
                  )}
                </div>
              ))}
          ...
          <div className="mb-6" data-section>
            ...
            <div className="flex flex-wrap gap-2">
              {resume.languages.map(lang => (
                <div key={lang.id} className="flex items-center gap-2 rounded-full px-3 py-1" style={{ border: `1px solid ${ACCENT}50` }}>
