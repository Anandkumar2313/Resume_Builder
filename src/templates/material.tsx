...
export function MaterialTemplate({ resume }: TemplateProps) {
  ...
  const sections = resume.visibleSections
  ...
  return (
    <div className="mx-auto w-full flex-1 bg-white shadow-lg" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      {/* Body — render sections in order, show heading always, content only when data exists */}
      <div className="p-8">
        {sections.map(key => {
          if (key === 'summary') return (
            <div key={key} className="mb-6" data-section>
              <SH title="Summary" />
              {pi.summary && <p className="px-3 text-sm leading-relaxed text-zinc-600" dangerouslySetInnerHTML={{ __html: md(pi.summary) }} />}
            </div>
          )
